...
export function authorization(fn: (req: NextApiRequestWithUser, res: NextApiResponse) => Promise<void>) {
  return async (req: NextApiRequestWithUser, res: NextApiResponse) => {
    ...
    try {
      authData = verify(process.env.SECRET_KEY, token) as UserAuthSafe
    } catch (error) {
      return res.status(401).json({ error: 'Unauthorized' })
    }
    req.user = authData
